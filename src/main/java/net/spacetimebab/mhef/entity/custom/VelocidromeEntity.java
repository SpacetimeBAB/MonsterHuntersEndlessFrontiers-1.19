...
import software.bernie.geckolib3.core.manager.AnimationFactory;
    ...
    public class VelocidromeEntity extends Monster implements IAnimatable {
        private AnimationFactory factory = new AnimationFactory(this);
        public VelocidromeEntity(EntityType<? extends Monster> entityType, Level level) {
        ...
        private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
            if (event.isMoving()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("velocidrome_Walk", true));
                return PlayState.CONTINUE;
            }
            if (this.isAggressive() && !(this.dead || this.getHealth() < 0.01 || this.isDeadOrDying())) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation("velocidrome_attack1", true));
                return PlayState.CONTINUE;
            }


            event.getController().setAnimation(new AnimationBuilder().addAnimation("velocidrome_Idle", true));
            return PlayState.CONTINUE;
        }
